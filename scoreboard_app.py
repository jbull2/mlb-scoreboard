...
# Safer path handling (always relative to the app file directory)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
CSV_PATH = os.path.join(BASE_DIR, "2025-08-04_predictions.csv")

# Check for CSV and load
if os.path.isfile(CSV_PATH):
    df_future_games = pd.read_csv(CSV_PATH)
else:
    st.warning("Predictions CSV not found. Please upload it.")
    uploaded_file = st.file_uploader("Upload your predictions CSV", type=["csv"])
    if uploaded_file is not None:
        df_future_games = pd.read_csv(uploaded_file)
    else:
        st.stop()  # Stop app until CSV is provided
